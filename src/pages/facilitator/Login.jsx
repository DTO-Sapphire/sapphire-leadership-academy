...
export default function FacilitatorLogin() {
  ...
  const verifyOtp = async e => {
    ...
    try {
      ...
      const { data: fac } = await supabase.from('facilitators').select('*').eq('email', email).single()
      if (!fac) { toast.error('Access denied. This email is not registered as a facilitator.'); await supabase.auth.signOut(); return }
      if (!fac.user_id) await supabase.from('facilitators').update({ user_id: user.id }).eq('id', fac.id)
    ...
    <div className="min-h-screen bg-[#0F52BA] flex items-center justify-center px-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-8">
        <div className="text-center mb-6">
          ...
          <h1 className="text-2xl font-bold text-[#0F52BA] mt-1">Facilitator Access</h1>
        ...
        {step === 'email' ? (
          <form onSubmit={sendOtp} className="space-y-4">
            <div>
              <label className="label">EXCO Email Address</label>
              <input className="input" type="email" required value={email}
                onChange={e => setEmail(e.target.value)} placeholder="exco@sapphirevirtualnetworks.com" />
            </div>
        ...
        <div className="mt-4 text-center">
          <Link to="/" className="text-xs text-gray-400 hover:text-gray-600">← Back to main site</Link>
        </div>
      </div>
    </div>
  )
